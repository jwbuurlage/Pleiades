...
tasks(bulk::world& world, const tpt::geometry::base<3_D, T>& g,
    ...
    auto scatters = std::vector<std::vector<scatter_task>>(p);
    auto gathers = std::vector<std::vector<gather_task>>(p);
    ...
    for (auto i = 0, J = s; J < g.projection_count(); J += p, i += 1) {
        ...
        for (auto& face : faces[i]) {
            ...
            auto pixels = std::accumulate(
                ...
                [](auto total, auto line) { return total + line.count; });
    ...
    // TODO nested std vectors not supported...
    // struct gather_task {
    //   int owner;
    //   std::vector<std::pair<std::size_t, scanline>> lines;
    // };
    ...
    auto gq =
        bulk::queue<int, std::vector<std::pair<std::size_t, scanline>>>(world);
    auto sq =
        bulk::queue<std::vector<int>,
                    std::vector<std::pair<std::vector<std::size_t>, scanline>>>(
            world);

    for (int t = 0; t < p; ++t) {
        for (auto task : scatters[t]) {
            sq(t).send(task.owner, task.lines)
        }
        for (auto task : gathers[t]) {
            gq(t).send(task.contributors, task.lines)
        }
    }
    ...
    local_gathers.resize(gq.size());
    local_scatters.resize(sq.size());

    for (auto [owner, lines] : gq) {
        local_gathers.push_back({owner, lines});
    }
    for (auto [contributors, lines] : sq) {
        local_scatters.push_back({contributors, lines});
    }
